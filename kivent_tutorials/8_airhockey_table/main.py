...
class TestGame(Widget):
    ...
    def setup_collision_callbacks(self):
        ...
        physics_system.add_collision_handler(
            1, 2,
            begin_func=self.begin_collide_with_wall)#low_collide_sound)#puck-wall
        physics_system.add_collision_handler(
            1, 8,
            begin_func=self.begin_collide_with_wall)#low_collide_sound)#puck-wall
        physics_system.add_collision_handler(
        ...
        physics_system.add_collision_handler(
            6, 2,
            begin_func=self.begin_collide_with_wall)#self.low_collide_sound)#paddle-wall
        physics_system.add_collision_handler(
            6, 8,
            begin_func=self.begin_collide_with_wall)#self.low_collide_sound)#paddle-wall
        physics_system.add_collision_handler(
    ...
    def begin_collide_with_wall(self, space, arbiter):
        ...
        vol = min(1, body1.velocity.length*.001)
        if vol<0.01:return
        ent = self.gameworld.entities[ent1_id]
        color = ent.color
        if arbiter.is_first_contact:
        ...
        lerp_system.add_lerp_to_entity(ent2_id, 'color', 'r', color.r, .2,
            'float')
        return arbiter.shapes[1].body.angle!=0.
    ...
    def lerp_callback_airhole(self, entity_id, component_name, property_name, 
        ...
        systems = self.gameworld.systems
        lerp_system = systems['lerp_system']
        lerp_system.add_lerp_to_entity(entity_id, 'color', 'a', self.faded_air_hole_alpha, 5.5,
            'float')
    ...
    def begin_seperate_with_airhole(self, space, arbiter):
        ...
        lerp_system = systems['lerp_system']
        lerp_system.clear_lerps_from_entity(ent2_id)
        lerp_system.add_lerp_to_entity(ent2_id, 'color', 'a', self.faded_air_hole_alpha, .75,
            'float')
    ...
    def draw_some_stuff(self):
        ...
        x1 = 225
        y1 = 95
        self.faded_air_hole_alpha = settingsDict['faded_air_hole_alpha']
    ...
    def create_air_triangle(self, pos,radius=60, collision_radius=0.001):
        ...
        col_shapes = [col_shape]
        color=(.25, .25, .25, self.faded_air_hole_alpha)
    ...
    def create_air_hole(self, pos,radius=60, collision_radius=0.001):
        ...
        col_shape = {'shape_type': 'circle', 'elasticity': .5, 
            'collision_type': 3, 'shape_info': shape_dict, 'friction': 1.0}
        col_shapes = [col_shape]
        color=(.25, .75, .25, self.faded_air_hole_alpha)
